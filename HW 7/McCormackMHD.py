...
First, mchalf calculates the "half-step" value.
Then, combined with the calculated "half-step" flux, the full step value is obtained.'''
def mchalf(C,i,old,flux):
    half = np.zeros(len(old))
    half[i] = old[i-1] - C*(flux[i] + -1*flux[i-1])
    if i == 0:
        half[i] = old[-1] - C*(flux[0] + -1*flux[-1]) #Periodic boundary
    return half
    
def mcc(C,i,old,half,halfflux):
    new = np.zeros(len(half))
    new[i] = 0.5*(old[i] + half[i]) - 0.5*C * (halfflux[i] + -1*halfflux[i-1])
    if i == 0:
        new[i] = 0.5*(old[0] +half[0]) - 0.5*C * (halfflux[0] + -1*halfflux[-1]) #Periodic boundary
    return new
...
for i in range(len(zarr)):
    ...
    '''Half-step values'''
    rhohalf[i] = mchalf(c,i,dens,massflux)
    momhalf[:,i] = mchalf(c,i,mom[0],Zmomflux)
    maghalf[:,i] = mchalf(c,i,mag[2],magflux)
    nrghalf[i] = mchalf(c,i,nrgarr,nrgflux)
    ...
    '''New values'''
    newrho = mcc(c,i,dens,rhohalf,halfFmass)
    newmom = mcc(c,i,mom[0],momhalf,halfFmom)
    newmag = np.zeros(shape=(3,len(zarr)))
    newmag[2] = mcc(c,i,mag[2],maghalf[2],halfFmag)
    newnrg = mcc(c,i,nrgarr,nrghalf,halfFnrg)
